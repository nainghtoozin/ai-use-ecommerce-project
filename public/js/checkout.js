document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentDiscount = typeof discountAmount !== 'undefined' ? discountAmount : 0;
    ...
    // --- Apply Coupon on page load if already applied ---
    if (appliedCoupon && appliedCoupon.code) {
        currentDiscount = appliedCoupon.discount || 0;
        document.getElementById('discountRow').style.display = 'flex';
        document.getElementById('orderDiscount').textContent = '-' + currentDiscount.toLocaleString() + ' MMK';
        appliedCouponInfo.style.display = 'block';
        appliedCouponText.textContent = appliedCoupon.name + ' (' + appliedCoupon.code + ')';
    ...
    applyCouponBtn.addEventListener('click', async function() {
        ...
        couponMessage.className = 'small text-muted';
        couponMessage.textContent = 'Applying...';
        ...
        try {
            const response = await fetch('/cart/apply-coupon', {
                ...
                body: JSON.stringify({ code, delivery_fee: currentDeliveryFee || 0 })
            });
            ...
            if (response.ok) {
                ...
                orderDiscountEl.textContent = '-' + currentDiscount.toLocaleString() + ' MMK';
                couponMessage.className = 'small text-success';
                couponMessage.textContent = '';
                appliedCouponInfo.style.display = 'block';
                appliedCouponText.textContent = result.coupon_name + ' (' + result.coupon_code + ')';
                couponCodeInput.disabled = true;
                applyCouponBtn.disabled = true;
                recalculateTotal();
            } else {
    ...
    removeCouponBtn.addEventListener('click', async function() {
        try {
            await fetch('/cart/remove-coupon', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                }
