...
export default function PlanPicker({ plans, subscription, currentPlan, interval, onIntervalChange, onChoose, recommendedSlug, allFeatureDefs = [], trialDays = null }) {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {plans.map((plan) => {
                        ...
                        return (
                            <div
                                key={plan.slug}
                                className={`relative rounded-xl border-2 p-5 pt-6 flex flex-col h-full transition-shadow ${isCurrent ? 'border-blue-500 bg-blue-50/30 dark:bg-blue-900/10 shadow-md shadow-blue-500/10' : 'border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 hover:shadow-md hover:border-gray-300 dark:hover:border-gray-700'}`}
                            >
                                <div className="absolute -top-3 left-1/2 -translate-x-1/2 z-10 flex gap-1">
                                    ...
                                    {!isCurrent && isFree && (
                                        <span className="px-3 py-1 text-xs font-semibold text-emerald-700 bg-emerald-100 rounded-full whitespace-nowrap">Free Trial</span>
                                    )}
                                    {!isCurrent && !isFree && recommendedSlug === plan.slug && (
                                ...
                                {plan.description && (
                                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 line-clamp-2 min-h-[2rem]">{plan.description}</p>
                                )}
                                ...
                                {isFree ? (
                                    <p className="text-[11px] text-emerald-600 font-medium mt-1">{trialDays ? `${trialDays}-day free trial included` : 'Free trial included'}</p>
                                ) : savings > 0 ? (
                                    <p className="text-[11px] text-emerald-600 font-medium mt-1">Save {formatCurrency(savings, pc)}/yr</p>
                                ) : (
                                    <p className="text-[11px] text-gray-400 mt-1">Billed {interval === 'yearly' ? 'annually' : 'monthly'}</p>
                                )}

                                <ul className="flex-1 mt-4 mb-5 pt-4 space-y-1.5 border-t border-gray-100 dark:border-gray-800">
                                ...
                                >
                                    {isCurrent ? 'Current Plan' : isFree ? 'Start Free Trial' : action === 'upgrade' ? 'Upgrade' : action === 'switch' ? 'Switch Plan' : 'Choose'}
                                </button>
                            </div>
                        );
                    })}
