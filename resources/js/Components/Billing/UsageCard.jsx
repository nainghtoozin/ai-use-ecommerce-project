export default function UsageCard({ label, current, limit, isUnlimited, format }) {
    ...
    return (
        <div className={`rounded-xl border p-5 ${percent >= 90 ? 'border-red-200' : percent >= 70 ? 'border-amber-200' : 'border-gray-200 dark:border-gray-800'} ${bgColor}`}>
            <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-semibold text-gray-800 dark:text-gray-200">{label}</span>
                <span className={`text-sm font-bold ${textColor}`}>
                    {displayCurrent}
                    <span className="text-gray-400 dark:text-gray-500 font-normal"> / {displayLimit}</span>
                </span>
            </div>
            <div className="w-full h-2.5 bg-white dark:bg-gray-900 rounded-full overflow-hidden" role="progressbar" aria-valuenow={percent} aria-valuemin={0} aria-valuemax={100} aria-label={`${label}: ${current} of ${limit ?? 'unlimited'}`}>
                {isUnlimited ? (
            ...
            <div className="mt-2 flex items-center justify-between">
                {isUnlimited ? (
                    ...
                    <span className={`text-xs font-medium ${percent >= 70 ? textColor : 'text-gray-500 dark:text-gray-400'}`}>
                        {remaining} remaining
                    </span>
                ) : null}
                {!isUnlimited && limit > 0 && (
