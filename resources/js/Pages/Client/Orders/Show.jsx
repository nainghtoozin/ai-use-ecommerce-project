                                    ...
                                    <table className="min-w-full">
                                        ...
                                        <tfoot>
                                            <tr>
                                                ...
                                                <td className="text-right py-2 text-sm text-gray-900 dark:text-gray-100">{formatCurrency(order.subtotal || order.items_total, cc)}</td>
                                            </tr>
                                            <tr>
                                                <td colSpan="3" className="text-right py-2 text-sm text-gray-600 dark:text-gray-400">Delivery Fee</td>
                                                <td className="text-right py-2 text-sm text-gray-900 dark:text-gray-100">{formatCurrency(order.delivery_fee || 0, cc)}</td>
                                            </tr>
                                            <tr className="font-bold">
                                                <td colSpan="3" className="text-right py-2 text-sm text-gray-900 dark:text-gray-100">Total</td>
                                                <td className="text-right py-2 text-sm text-gray-900 dark:text-gray-100">{formatCurrency(order.total_amount, cc)}</td>
                                            </tr>
                                        </tfoot>
                                    </table>
                                </div>
                                ...
                                <div className="p-6">
                                    ...
                                    <form onSubmit={handleUploadPayment} encType="multipart/form-data">
                                        <div className="mb-4">
                                            ...
                                            <input type="text" value={data.transaction_id} onChange={(e) => setData('transaction_id', e.target.value)} className="w-full border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                        </div>
                                        <div className="mb-4">
                                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Payment Proof</label>
                                            <input type="file" onChange={(e) => setData('payment_proof', e.target.files[0])} accept="image/*" required className="w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" />
                                        </div>
                    ...
                    <div className="space-y-6">
                        <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-6">
                            ...
                            <div className="space-y-3">
                                <div>
                                    ...
                                    <span className={`inline-block mt-1 px-2.5 py-0.5 rounded-full text-xs font-medium ${orderStatusColors[order.order_status] || 'bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200'}`}>
                                        {order.order_status}
                                    </span>
                                </div>
                                <div>
                            ...
                            <div className="space-y-3">
                                ...
                                {order.transaction_id && (
                                    <div>
                                        ...
                                        <p className="font-medium text-gray-900 dark:text-gray-100">{order.transaction_id}</p>
                                    </div>
                                )}
                                {order.payment_screenshot && (
                                    <div>
                                        ...
                                        <a href={order.payment_screenshot_url} target="_blank" rel="noopener noreferrer">
                                            <img src={order.payment_screenshot_url} alt="Payment Screenshot" className="w-40 h-40 rounded-lg border border-gray-200 dark:border-gray-800 object-cover hover:opacity-90 transition-opacity" />
                                        </a>
                                    </div>
                                )}
                                {order.paid_amount && (
                        ...
                        <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-6">
                            ...
                            <div className="space-y-3">
                                ...
                                {(cityLabel || townshipLabel) && (
                                    <div>
                                        <p className="text-sm text-gray-500 dark:text-gray-400">City / Township</p>
                                        <p className="font-medium text-gray-900 dark:text-gray-100">{cityLabel}{townshipLabel ? `, ${townshipLabel}` : ''}</p>
                                    </div>
                                )}
                                {order.postal_code && (
                                    <div>
                                        <p className="text-sm text-gray-500 dark:text-gray-400">Postal Code</p>
                                        <p className="font-medium text-gray-900 dark:text-gray-100">{order.postal_code}</p>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-6">
                            ...
                            {order.can_cancel ? (
                                ...
                                <p className="text-sm text-gray-500 dark:text-gray-400">No actions available for this order.</p>
                            )}
                        </div>
                    </div>
                </div>
