            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 mb-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                        ...
                        <div>
                            ...
                            <select value={orderStatus} onChange={e => { setOrderStatus(e.target.value); applyFilters({ order_status: e.target.value }); }} className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        <div>
                            ...
                            <select value={paymentStatus} onChange={e => { setPaymentStatus(e.target.value); applyFilters({ payment_status: e.target.value }); }} className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        <div>
                            ...
                            <select value={dateRange} onChange={e => { const v = e.target.value; setDateRange(v); setShowCustomDate(v === 'custom'); if (v !== 'custom') applyFilters({ date_range: v }); }} className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        <div>
                            ...
                            <select value={sort} onChange={e => { setSort(e.target.value); applyFilters({ sort: e.target.value }); }} className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    {showCustomDate && (
                        <div className="grid grid-cols-2 gap-3 mt-3">
                            <div>
                                ...
                                <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-gray-500 mb-1">To</label>
                                <div className="flex gap-2">
                                    <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className="flex-1 border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                ...
                {!orders?.data?.length ? (
                    <div className="text-center py-16 bg-white rounded-2xl border border-gray-200 shadow-sm">
                        ...
                        <h3 className="mt-4 text-xl font-semibold text-gray-900">{hasActiveFilters ? 'No orders match your filters' : 'No orders yet'}</h3>
                        <p className="mt-2 text-base text-gray-500">{hasActiveFilters ? 'Try adjusting your search or filter criteria.' : 'Your order history will appear here once you make a purchase.'}</p>
                        {hasActiveFilters ? (
                    ...
                    <div className="space-y-4">
                        <p className="text-sm text-gray-500">{orders.total} order{orders.total !== 1 ? 's' : ''} found</p>
                        {orders.data.map((order) => (
                            <div key={order.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow overflow-hidden">
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                                        <div className="min-w-0">
                                            <div className="flex items-center gap-2.5 flex-wrap">
                                                <h3 className="text-base font-bold text-gray-900 font-mono">{order.invoice_number}</h3>
                                                <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${statusColors[order.order_status] || 'bg-gray-50 text-gray-600 border border-gray-200'}`}>
                                                    {order.order_status}
                                                </span>
                                                <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${paymentColors[order.payment_status] || 'bg-gray-50 text-gray-600 border border-gray-200'}`}>
                                        ...
                                        <div className="text-left sm:text-right shrink-0">
                                            <p className="text-xl font-bold text-gray-900">{formatCurrency(order.total_amount, cc)}</p>
                                        </div>
                                    </div>
                                    ...
                                    {order.items?.length > 0 && (
                                        <div className="mt-3 pt-3 border-t border-gray-100 flex gap-2 overflow-hidden">
                                            {order.items.slice(0, 4).map((item) => (
                                                <span key={item.id} className="inline-flex items-center gap-1.5 bg-gray-50 px-2.5 py-1 rounded-lg text-sm text-gray-600">
                                                    <span className="font-medium text-gray-800 truncate max-w-[120px]">{item.product?.name || 'Product'}</span>
                                                    <span className="text-gray-400">&times;{item.quantity}</span>
                                                </span>
                                            ))}
                                            {order.items.length > 4 && (
                                                <span className="inline-flex items-center text-sm text-gray-500 px-1">+{order.items.length - 4} more</span>
                                            )}
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="px-5 py-3 bg-gray-50 border-t border-gray-100 flex items-center gap-2">
                                    ...
                                    <a href={route('storefront.customer.orders.invoice', { store_slug: storeSlug, invoice_number: order.invoice_number })} target="_blank" className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">
                        ...
                        {orders.links?.length > 3 && (
                            <div className="flex justify-center gap-1 pt-4">
                                {orders.links.map((link, i) => (
                                    <Link key={i} href={link.url || '#'} preserveState className={`px-4 py-2 text-sm rounded-xl transition-colors ${link.active ? 'bg-blue-600 text-white font-medium' : link.url ? 'text-gray-700 hover:bg-gray-100' : 'text-gray-400 cursor-not-allowed'}`}>
