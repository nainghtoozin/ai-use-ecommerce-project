...
export default function Orders({ tenant, orders }) {
                ...
                {!orders?.data?.length ? (
                    <div className="text-center py-12 sm:py-16 bg-white rounded-xl border border-gray-200">
                        ...
                        <Link
                            ...
                            className="mt-6 inline-block px-6 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700"
                        >
                            Start Shopping
                        </Link>
                    </div>
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-500 mt-1">
                                            {new Date(order.created_at).toLocaleDateString()} &middot; {order.items?.reduce((s, i) => s + i.quantity, 0)} items
                                        </p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-lg font-bold text-gray-900">{formatCurrency(order.total_amount, cc)}</p>
                                        <p className="text-sm text-gray-500">{order.payment_method?.name || ''}</p>
                                    </div>
                                </div>
