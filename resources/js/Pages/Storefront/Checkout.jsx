...
export default function StorefrontCheckout({ tenant, cartItems, subtotal, paymentMethods, cities, errors, appliedPromotion: initialAppliedPromotion, discountAmount: initialDiscountAmount, autoPromotions, addresses = [], defaultAddress = null }) {
    ...
    useEffect(() => {
        if (defaultAddress) {
            ...
            if (defaultAddress.city_id) {
                fetchTownships(defaultAddress.city_id).then((data) => {
                    if (defaultAddress.township_id) {
                        const t = data.find(tw => tw.id == defaultAddress.township_id);
                        if (t?.postal_code) {
                            setForm((prev) => ({ ...prev, postal_code: t.postal_code }));
                        }
                    }
                });
            }
    ...
    function selectAddress(address) {
        ...
        if (address.city_id) {
            fetchTownships(address.city_id).then((data) => {
                if (address.township_id) {
                    const t = data.find(tw => tw.id == address.township_id);
                    if (t?.postal_code) {
                        setForm((prev) => ({ ...prev, postal_code: t.postal_code }));
                    }
                }
    ...
    function handleSubmit(e) {
        ...
        router.post(`/store/${tenant.slug}/checkout`, form, {
            preserveScroll: true,
            onError: (errs) => {
                clearInterval(progressInterval);
                setUploadProgress(0);
            ...
            <ShopLayout>
                <div className="max-w-md mx-auto px-4 py-16 text-center">
                    <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-8">
                        ...
                        <Link href={`/store/${tenant.slug}/login`} className="inline-block w-full px-6 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium">
                            Sign In
                        </Link>
                        <Link href={`/store/${tenant.slug}/cart`} className="block mt-3 text-sm text-gray-500 hover:text-gray-700 dark:text-gray-300">
                            &larr; Back to Cart
                        </Link>
                    </div>
                </div>
    ...
    return (
        <ShopLayout>
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="mb-4">
                    <Link href={`/store/${tenant.slug}/cart`} className="text-sm text-blue-600 hover:text-blue-800 inline-block">
                        &larr; Back to Cart
                    </Link>
                </div>
                ...
                <form onSubmit={handleSubmit}>
                    {step === 1 && (
                        <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-6 space-y-4">
                            {addresses.length > 0 && (
                                <div className="mb-4 p-4 bg-gray-50 dark:bg-gray-950 rounded-lg border border-gray-200 dark:border-gray-800">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Delivering to:</span>
                                            {selectedAddress ? (
                                                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                                    {selectedAddress.first_name} {selectedAddress.last_name} — {selectedAddress.address_line}, {selectedAddress.city?.name || ''}
                                                </p>
                                            ) : (
                                                <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">No address selected</p>
                                            )}
                                        </div>
                                        ...
                                        <div className="mt-3 space-y-2 border-t pt-3">
                                            {addresses.map((addr) => (
                                                ...
                                                >
                                                    ...
                                                    <p className="text-gray-600 dark:text-gray-400 mt-1">{addr.address_line}</p>
                                                    <p className="text-gray-500 dark:text-gray-400">{addr.first_name} {addr.last_name} — {addr.phone}</p>
                                                </button>
                                            ))}
                                        </div>
                                    )}
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-3">
                                {paymentMethods?.map((pm) => {
                                    ...
                                    return (
                                        <div key={pm.id} onClick={() => updateField('payment_method_id', pm.id)}
                                            className={`rounded-xl border-2 transition-all cursor-pointer select-none ${isSelected ? 'border-blue-500 bg-blue-50/50 shadow-sm' : 'border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 hover:border-gray-300'}`}>
                                            <div className="p-4">
                                                <div className="flex items-center gap-3">
                                                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${isSelected ? 'border-blue-500' : 'border-gray-300 dark:border-gray-700'}`}>
                                                                        ...
                                                                        <div className="space-y-3">
                                                                            ...
                                                                            <div>
                                                                                ...
                                                                                <div className="flex items-center gap-2 mt-0.5">
                                                                                    ...
                                                                                    {pm.account_number && (
                                                                                        <button type="button" onClick={(e) => { e.stopPropagation(); copyToClipboard(pm.account_number, pm.id); }}
                                                                                            className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-md transition-colors ${copiedId === pm.id ? 'bg-green-100 text-green-700' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200'}`}>
                                                                                            {copiedId === pm.id ? <><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg> Copied</> : <><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg> Copy</>}
                                                                                        </button>
                                                                                    )}
                                                                                </div>
                                                                            </div>
                            ...
                            <div className="lg:col-span-2 lg:sticky lg:top-24">
                                <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 p-6">
                                    ...
                                    <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-800 space-y-3">
                                                ...
                                                <div className="flex items-center justify-between">
                                                    <div className="min-w-0 flex-1">
                                                        ...
                                                        <div className="flex items-center gap-2 mt-1 ml-5">
                                                            ...
                                                            <button type="button" onClick={() => copyToClipboard(localAppliedPromotion.code, 'promo')}
                                        ...
                                        {promoMessage && (
                                            <div className={`text-xs px-3 py-2 rounded-lg flex items-center gap-1.5 transition-all duration-300 ${promoError ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-emerald-50 text-emerald-700 border border-emerald-200'}`}>
                                                <svg className="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    {promoError ? <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /> : <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />}
                                                </svg>
                                                {promoMessage}
                                            </div>
                                        )}
                                    ...
                                    <div className="mt-6 space-y-3">
                                        ...
                                        {uploading && (
                                            <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
                                                <div className="bg-green-500 h-full rounded-full transition-all duration-300 ease-out"
                ...
                {qrPreview && (
                    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4" onClick={() => setQrPreview(null)}>
                        <div className="relative max-w-md w-full" onClick={(e) => e.stopPropagation()}>
                            ...
                            <img src={qrPreview} alt="QR Code Full Size" className="w-full rounded-lg shadow-2xl" />
                        </div>
                    </div>
                )}
            </div>
