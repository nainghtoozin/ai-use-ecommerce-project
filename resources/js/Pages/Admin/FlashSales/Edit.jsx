...
export default function FlashSaleEdit({ flashSale, products }) {
    const { data, setData, put, processing, errors } = useForm({
        name: flashSale.name || '',
        description: flashSale.description || '',
        discount_type: flashSale.discount_type || 'percentage',
        discount_value: flashSale.discount_value ?? 0,
        max_discount_amount: flashSale.max_discount_amount ?? '',
        starts_at: toDatetimeLocal(flashSale.starts_at),
        ends_at: toDatetimeLocal(flashSale.ends_at),
        is_active: flashSale.is_active ?? true,
        priority: flashSale.priority ?? 0,
        usage_limit: flashSale.usage_limit ?? '',
        products: (flashSale.flash_sale_products || []).map(fsp => ({
            product_id: fsp.product_id,
            product_name: fsp.product?.name || `Product #${fsp.product_id}`,
            original_price: fsp.product?.price || 0,
            variant_id: fsp.variant_id,
    ...
    const filteredProducts = productSearch
        ? products.filter(p => p.name.toLowerCase().includes(productSearch.toLowerCase()) && !data.products.some(fp => fp.product_id === p.id))
        : products.filter(p => !data.products.some(fp => fp.product_id === p.id));
    ...
    function handleSubmit(e) {
        e.preventDefault();
        put(adminUrl(`/admin/flash-sales/${flashSale.id}`));
    }
        ...
        <AdminLayout>
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div className="mb-6">
                    ...
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mt-2">Edit Flash Sale</h1>
                    <p className="text-sm text-gray-500 mt-1">Editing: <span className="font-medium">{flashSale.name}</span></p>
                </div>
                                    ...
                                    <div className="max-h-48 overflow-y-auto divide-y divide-gray-50">
                                        {filteredProducts.length === 0 ? (
                                            ...
                                            <button key={product.id} type="button" onClick={() => addProduct(product)}
                                                className="w-full flex items-center justify-between px-4 py-2.5 text-left hover:bg-purple-50/50 cursor-pointer transition-colors">
