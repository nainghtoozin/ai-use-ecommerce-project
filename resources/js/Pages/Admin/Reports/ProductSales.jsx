    ...
    return (
        <tr className="hover:bg-gray-50 dark:bg-gray-950 transition-colors">
            <td className="px-3 sm:px-5 py-3 sm:py-3.5">
                <span className="text-xs sm:text-sm font-mono text-gray-500 dark:text-gray-400">#{item.product_id}</span>
            </td>
            <td className="px-3 sm:px-5 py-3 sm:py-3.5">
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs sm:text-sm font-medium text-gray-900 dark:text-gray-100 truncate max-w-[200px]">
                        {item.product_name}
                    </span>
                </div>
            </td>
            <td className="px-3 sm:px-5 py-3 sm:py-3.5 text-xs sm:text-sm text-gray-600 dark:text-gray-400">
...
function FilterForm({ form, setForm, onSubmit, onReset, categories }) {
    return (
        <form onSubmit={onSubmit} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-4 lg:p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 lg:gap-4">
                ...
                <div>
                    <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Stock Status</label>
            ...
            <div className="flex items-center gap-2">
                ...
                <button type="button" onClick={onReset}
                    className="px-3 py-2 text-sm text-gray-500 hover:text-gray-700 dark:text-gray-300 transition-colors">
                    Reset
                </button>
            </div>
        </form>
    ...
    return (
        <AdminLayout>
            ...
            <div className="max-w-[1600px] mx-auto px-4 sm:px-5 lg:px-6 py-6 lg:py-8 space-y-6">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Product Sales Report</h1>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Product-level sales performance with inventory visibility</p>
                </div>
                ...
                <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 sm:px-5 py-4 border-b border-gray-100 dark:border-gray-800">
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Product Sales</h2>
                        </div>
                        <div className="flex items-center gap-2">
                        ...
                        <table className="w-full">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {!products?.data?.length ? (
                                    <tr>
                                        <td colSpan="8" className="px-5 py-16 text-center text-gray-400 dark:text-gray-500">
                    ...
                    <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
                        <div className="px-4 sm:px-5 py-4 border-b border-gray-100 dark:border-gray-800">
                            <div className="flex items-center gap-2">
                                ...
                                <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Top Selling Products</h2>
                            </div>
                            <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Highest units sold in selected period</p>
                        </div>
                        <div className="divide-y divide-gray-100">
                            {(!top_selling || top_selling.length === 0) ? (
                                <div className="px-5 py-10 text-center text-gray-400 dark:text-gray-500 text-sm">No data for this period.</div>
                            ) : (
                                top_selling.map((item, i) => (
                                    <div key={item.product_id} className="flex items-center gap-3 sm:gap-4 px-4 sm:px-5 py-3 hover:bg-gray-50 dark:bg-gray-950 transition-colors">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-xs text-gray-400 dark:text-gray-500">{Number(item.qty_sold).toLocaleString()} units &middot; {formatCurrency(item.revenue)}</p>
                                        </div>
                                        <StockBadge stock={item.stock} />
                                    </div>
                                ))
                    ...
                    <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
                        <div className="px-4 sm:px-5 py-4 border-b border-gray-100 dark:border-gray-800">
                            <div className="flex items-center gap-2">
                                ...
                                <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Slow Moving Products</h2>
                            </div>
                            <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">In-stock products with no sales in selected period</p>
                        </div>
                        <div className="divide-y divide-gray-100">
                            {(!slow_moving || slow_moving.length === 0) ? (
                                <div className="px-5 py-10 text-center text-gray-400 dark:text-gray-500 text-sm">No slow moving products found.</div>
                            ) : (
                                slow_moving.map((item) => (
                                    <div key={item.id} className="flex items-center gap-3 sm:gap-4 px-4 sm:px-5 py-3 hover:bg-gray-50 dark:bg-gray-950 transition-colors">
                                        <div className="p-1.5 rounded-lg bg-amber-50 shrink-0">
                                            <Package className="w-4 h-4 text-amber-600" />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <p className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{item.name}</p>
                                            <p className="text-xs text-gray-400 dark:text-gray-500">Stock: {Number(item.stock).toLocaleString()} &middot; {formatCurrency(item.price)}</p>
                                        </div>
                                        <StockBadge stock={item.stock} />
                                    </div>
                                ))
