...
export default function AdminNotificationsIndex({ notifications, unread_count }) {
    ...
    const isOperational = (title) => {
        if (!title) return false;
        return title.includes('New Order') || title.includes('Proof') || title.includes('Payment') || title.includes('Stock') || title.includes('Low Stock') || title.includes('Out of Stock');
    };

    const getOrderId = (n) => n.order_id || n.data?.order_id;
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                    ...
                    {effectiveUnread > 0 && (
                        <button
                            ...
                            disabled={markingAll}
                            className="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-blue-700 bg-blue-50 rounded-xl hover:bg-blue-100 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-150 active:scale-[0.98]"
                        >
                            <CheckIcon />
                            ...
                            {grouped.map(([label, items]) => (
                                <div key={label}>
                                    ...
                                    {items.map((notification, idx) => {
                                        ...
                                        const operational = isOperational(notification.title);
                                        return (
                                            <div
                                                key={notification.id}
