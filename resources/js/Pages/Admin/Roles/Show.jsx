...
export default function RolesShow({ role, grouped_permissions }) {
    ...
    return (
        <AdminLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">Role Details</h2>}>
                        ...
                        <div className="p-6">
                            <div className="flex items-start justify-between">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-500 dark:text-gray-400">Guard: {role.guard_name}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-2">
                                    ...
                                    {['superadmin', 'admin'].includes(role.name) && (
                                        <span className="px-4 py-2 text-sm text-gray-400 dark:text-gray-500 italic">Protected system role</span>
                                    )}
                                    ...
                                    {['superadmin', 'admin'].includes(role.name) && (
                                        <span className="px-4 py-2 text-sm text-gray-400 dark:text-gray-500 italic">System role — protected</span>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                                <div className="p-4 bg-blue-50 rounded-lg">
                                    ...
                                    <p className="text-gray-500 dark:text-gray-400">Permissions</p>
                                </div>
                                <div className="p-4 bg-green-50 rounded-lg">
                                    <p className="text-green-600 font-medium">{role.users_count}</p>
                                    <p className="text-gray-500 dark:text-gray-400">Assigned Users</p>
                                </div>
                                <div className="p-4 bg-gray-50 dark:bg-gray-950 rounded-lg">
                                    <p className="text-gray-600 dark:text-gray-400 font-medium">{new Date(role.created_at).toLocaleDateString()}</p>
                                ...
                                <div className="space-y-4">
                                    {grouped_permissions.map((group) => (
                                        <div key={group.group} className="border border-gray-200 dark:border-gray-800 rounded-lg overflow-hidden">
                                            <div className="bg-gray-50 dark:bg-gray-950 px-4 py-3">
                                                <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300">{group.label}</h4>
                                            </div>
