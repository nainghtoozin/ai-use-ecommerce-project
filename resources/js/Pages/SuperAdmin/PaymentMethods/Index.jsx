...
import { CreditCard, ChevronUp, ChevronDown, GripVertical } from 'lucide-react';

function sortableList(items) {
    return [...items].sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0));
}

export default function PaymentMethodsIndex({ paymentMethods, filters }) {
    const [search, setSearch] = useState(filters?.search || '');
    const [statusFilter, setStatusFilter] = useState(filters?.status || '');

    function handleSearch(e) {
        e.preventDefault();
        router.get('/superadmin/payment-methods', {
            search,
            status: statusFilter,
    ...
    function handleToggle(id) {
        router.post(`/superadmin/payment-methods/${id}/toggle`, {}, {
            preserveState: true,
            preserveScroll: true,
        });
    }
    ...
    function handleDelete(id) {
        if (window.confirm('Archive this payment method? It will be hidden from merchants.')) {
            router.delete(`/superadmin/payment-methods/${id}`, {
                preserveState: true,
                preserveScroll: true,
            });
        }
    ...
    function handleRestore(id) {
        router.post(`/superadmin/payment-methods/${id}/restore`, {}, {
            preserveState: true,
            preserveScroll: true,
        });
    }
                    ...
                    <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div className="p-6">
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                                ...
                                <Link
                                    href="/superadmin/payment-methods/create"
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                        {isArchived ? (
                                                            ...
                                                            <div className="flex items-center justify-end gap-2">
                                                                <Link
                                                                    href={`/superadmin/payment-methods/${pm.id}/edit`}
