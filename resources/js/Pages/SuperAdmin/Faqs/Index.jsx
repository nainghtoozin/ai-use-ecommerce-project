...
export default function FaqsIndex({ faqs, categories, filters }) {
    ...
    function handleToggle(faq) {
        router.post(`/superadmin/faqs/${faq.id}/toggle`, {}, { preserveState: true });
    }

    function handleDelete(faq) {
        const q = faq.question_en || 'this FAQ';
        if (window.confirm(`Delete "${q}"?`)) {
            router.delete(`/superadmin/faqs/${faq.id}`);
        }
    }
    ...
    function handleBulkAction() {
        ...
        router.post('/superadmin/faqs/bulk-action', { ids: selected, action: bulkAction }, {
            preserveState: true,
    ...
    function handleDragEnd() {
        ...
        router.post('/superadmin/faqs/reorder', { ids: items.map(i => i.id) }, { preserveState: true });
                    ...
                    <div className="bg-white dark:bg-gray-900 overflow-hidden shadow-sm sm:rounded-lg">
                        <div className="p-6">
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                                ...
                                <Link href="/superadmin/faqs/create" className="px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-lg hover:bg-green-700 transition-colors">+ Add FAQ</Link>
                            </div>
                            ...
                            <div className="overflow-x-auto">
                                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                                    ...
                                    <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-800">
                                            ...
                                            <tr key={faq.id} draggable onDragStart={() => handleDragStart(index)} onDragEnter={() => handleDragEnter(index)} onDragEnd={handleDragEnd} onDragOver={(e) => e.preventDefault()} className="hover:bg-gray-50 dark:hover:bg-gray-950 cursor-grab active:cursor-grabbing">
                                                ...
                                                <td className="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                                                    <div className="flex items-center justify-end gap-2">
                                                        <Link href={`/superadmin/faqs/${faq.id}/edit`} className="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">Edit</Link>
                                        ...
                                        {faqs.data.length === 0 && (
                                            <tr><td colSpan="7" className="px-6 py-12 text-center text-gray-500 dark:text-gray-400">No FAQs found.</td></tr>
                                        )}
                                    </tbody>
                                </table>
                            </div>
