...
function ConfirmDialog({ open, title, message, confirmLabel, confirmClass, onConfirm, onCancel, children }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-lg w-full mx-4 p-6" role="dialog" aria-modal="true">
                ...
                <div className="mt-6 flex gap-3 justify-center">
                    <button onClick={onCancel} className="px-5 py-2.5 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 rounded-xl text-sm font-medium hover:bg-gray-50 transition-colors">
                        Cancel
                    </button>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800">
            <div className="px-5 py-3 border-b border-gray-100 dark:border-gray-800">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                    <ShieldCheck className="w-4 h-4 text-gray-400 dark:text-gray-500" /> Review Payment
                </h3>
            </div>
            ...
            >
                <div className="mt-4">
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 text-left mb-1.5">Reason for rejection</label>
                    <textarea
                        value={rejectReason}
                        onChange={(e) => setRejectReason(e.target.value.slice(0, 2000))}
                        rows={3}
                        placeholder="Explain why this payment is being rejected..."
                        className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none resize-none"
                        aria-label="Rejection reason"
                    />
                    <p className="text-xs text-gray-400 dark:text-gray-500 mt-1 text-right">{rejectReason.length}/2000</p>
                </div>
            </ConfirmDialog>
        </div>
    );
...
function PaymentDetailDrawer({ intent, open, onClose, onApprove, onReject, processing }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex">
            ...
            <div className="relative ml-auto w-full max-w-xl bg-white dark:bg-gray-900 shadow-2xl overflow-y-auto" role="dialog" aria-modal="true" aria-label="Payment review details">
                <div className="sticky top-0 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-6 py-4 flex items-center justify-between z-10">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100">Payment Review</h2>
                ...
                <div className="p-6 space-y-6">
                    ...
                    {intent.tenant && (
                        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800">
                            <div className="px-5 py-3 border-b border-gray-100 dark:border-gray-800">
                                <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                                    <Store className="w-4 h-4 text-gray-400 dark:text-gray-500" /> Merchant Information
                                </h3>
                            </div>
                            <div className="p-5 space-y-2.5">
                                <div className="flex items-center justify-between text-sm">
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <span className="font-semibold text-gray-900 dark:text-gray-100">{intent.tenant.name}</span>
                                    </div>
                                </div>
                                <div className="flex items-center justify-between text-sm">
                                    <span className="text-gray-500 dark:text-gray-400">Slug</span>
                                    <span className="font-mono text-sm text-gray-600 dark:text-gray-400">{intent.tenant.slug}</span>
                                </div>
                                <div className="flex items-center justify-between text-sm">
                                    <span className="text-gray-500 dark:text-gray-400">Email</span>
                                    <span className="text-sm text-gray-600 dark:text-gray-400">{intent.tenant.email || '—'}</span>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="bg-gray-50 dark:bg-gray-950 rounded-xl p-5 space-y-3">
                        <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-1">Payment Details</h3>
                        <div className="flex items-center justify-between text-sm">
                        ...
                        <div className="flex items-center justify-between text-sm">
                            ...
                            <span className="font-semibold text-gray-900 dark:text-gray-100">{fmt(intent.amount, intent.currency)}</span>
                        </div>
                        <div className="flex items-center justify-between text-sm">
                        ...
                        <div className="flex items-center justify-between text-sm">
                            ...
                            <span className="font-semibold text-gray-900 dark:text-gray-100">{formatDateTime(intent.created_at)}</span>
                        </div>
                    </div>
                            ...
                            <div className="p-5 space-y-4">
                                {intent.evidences.map((ev) => (
                                    <div key={ev.id} className="space-y-3">
                                        <div className="bg-gray-50 dark:bg-gray-950 rounded-lg p-4 space-y-2.5">
                                            {ev.sender_name && (
                                                <div className="flex items-center justify-between text-sm">
                                                    <span className="text-gray-500 dark:text-gray-400">Sender Name</span>
                                                    <span className="font-semibold text-gray-900 dark:text-gray-100">{ev.sender_name}</span>
                                                </div>
                                            )}
                                            {ev.sender_account && (
                                                <div className="flex items-center justify-between text-sm">
                                                    <span className="text-gray-500 dark:text-gray-400">Account / Phone</span>
                                                    <span className="font-semibold text-gray-900 dark:text-gray-100">{ev.sender_account}</span>
                                                </div>
                                            )}
                                            {ev.transaction_reference && (
                                                <div className="flex items-center justify-between text-sm">
                                                    <span className="text-gray-500 dark:text-gray-400">Transaction Ref</span>
                                                    <span className="font-mono font-semibold text-gray-900 dark:text-gray-100">{ev.transaction_reference}</span>
                                                </div>
                                            )}
                                            {ev.transferred_amount && (
                                                <div className="flex items-center justify-between text-sm">
                                                    <span className="text-gray-500 dark:text-gray-400">Transferred Amount</span>
                                                    <span className="font-semibold text-gray-900 dark:text-gray-100">{fmt(ev.transferred_amount, intent.currency)}</span>
                                                </div>
                                            )}
                                            {ev.transfer_date && (
                                                <div className="flex items-center justify-between text-sm">
                                                    <span className="text-gray-500 dark:text-gray-400">Transfer Date</span>
                                                    <span className="font-semibold text-gray-900 dark:text-gray-100">{formatDate(ev.transfer_date)}</span>
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        {ev.note && <p className="text-xs text-gray-500 dark:text-gray-400 italic">"{ev.note}"</p>}
                                    </div>
                                ))}
                            </div>
                        </div>
        ...
        <AdminLayout>
            ...
            <div className="p-6 lg:p-8 space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Billing Console</h1>
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Review and manage all subscription payments across the platform</p>
                    </div>
                    {pendingPriority > 0 && (
                            ...
                            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                                <thead className="bg-gray-50 dark:bg-gray-950">
                                    <tr>
                                        ...
                                        <th className="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Submitted</th>
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {items.map((intent) => (
                                        <tr key={intent.id} className={`hover:bg-gray-50 dark:bg-gray-950/50 transition-colors ${intent.status === 'waiting_review' ? 'bg-purple-50/30' : ''}`}>
                                            ...
                                            <td className="px-5 py-4 whitespace-nowrap">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div>
                                                        <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{intent.tenant?.name || '—'}</p>
                                                        <p className="text-xs text-gray-400 dark:text-gray-500">{intent.tenant?.email || ''}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-5 py-4 whitespace-nowrap">
                                                <span className="text-sm text-gray-600 dark:text-gray-400">{intent.plan?.name || '—'}</span>
                                            </td>
                                            <td className="px-5 py-4 whitespace-nowrap">
                                                <span className="text-sm font-semibold text-gray-900 dark:text-gray-100">{fmt(intent.amount, intent.currency)}</span>
                                            </td>
                                            <td className="px-5 py-4 whitespace-nowrap">
