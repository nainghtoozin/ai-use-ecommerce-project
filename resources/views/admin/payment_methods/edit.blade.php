...
<div class="max-w-2xl mx-auto bg-white p-6 rounded-md shadow-md">
    ...
    <form action="{{ route('admin.payment-methods.update', $paymentMethod->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <div>
            <label for="qr_image" class="block text-gray-700 font-medium">QR Code Image <span class="text-gray-400">(Optional)</span></label>
            <input type="file" name="qr_image" id="qr_image" accept="image/jpg,image/jpeg,image/png,image/webp"
                   class="w-full border border-gray-300 rounded-md px-3 py-2 mt-1 focus:outline-none focus:ring focus:border-blue-400">
            @error('qr_image') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
            @if($paymentMethod->qr_image)
                <div class="mt-2">
                    <p class="text-xs text-gray-500 mb-1">Current QR:</p>
                    <img src="{{ image_url($paymentMethod->qr_image) }}" alt="QR Code" class="w-24 h-24 rounded border border-gray-200 object-cover">
                </div>
            @endif
        </div>
