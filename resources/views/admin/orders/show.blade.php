        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-md shadow p-6">
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        @if($order->paymentMethod)
                            @if($order->paymentMethod->account_name)
                                <p class="text-sm text-gray-500">Account: {{ $order->paymentMethod->account_name }}</p>
                            @endif
                            @if($order->paymentMethod->account_number)
                                <p class="text-sm text-gray-500">Number: {{ $order->paymentMethod->account_number }}</p>
                            @endif
                        @endif
                    </div>
                    ...
                    @if($order->payer_name)
                    <div>
                        <span class="text-gray-600 text-sm">Sender Name:</span>
                        <p class="font-medium">{{ $order->payer_name }}</p>
                    </div>
                    @endif
                    ...
                    <div>
                        <span class="text-gray-600 text-sm">Payment Screenshot:</span>
                        <div class="mt-2">
                            <a href="{{ $order->payment_screenshot_url }}" target="_blank" class="block">
                                <img src="{{ $order->payment_screenshot_url }}" alt="Payment Screenshot" class="max-w-full h-auto rounded-md border" style="max-height: 200px;">
                            </a>
                            <a href="{{ $order->payment_screenshot_url }}" target="_blank" class="text-blue-500 text-sm hover:underline mt-1 block">
                                View Full Image
                            </a>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="mt-2">
                            <a href="{{ $order->payment_proof_url }}" target="_blank" class="block">
                                <img src="{{ $order->payment_proof_url }}" alt="Payment Proof" class="max-w-full h-auto rounded-md border" style="max-height: 200px;">
                            </a>
                            <a href="{{ $order->payment_proof_url }}" target="_blank" class="text-blue-500 text-sm hover:underline mt-1 block">
                                View Full Image
                            </a>
                        </div>
                    </div>
...
<div id="rejectPaymentModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg shadow-lg p-6 w-96">
        ...
        <p class="text-sm text-gray-600 mb-4">Provide a reason for rejecting this payment (optional):</p>
        <form action="{{ route('admin.orders.reject-payment', $order->id) }}" method="POST">
            ...
            <div class="mb-4">
                <textarea name="rejection_reason" rows="3"
                          ...
                          placeholder="e.g. Screenshot is unclear, incorrect amount, etc."></textarea>
            </div>
            <div class="flex justify-end gap-3">
                <button type="button" 
                        onclick="document.getElementById('rejectPaymentModal').classList.add('hidden')"
